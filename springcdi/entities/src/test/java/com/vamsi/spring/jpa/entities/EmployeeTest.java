...
public class EmployeeTest {
	...
	public static void setEntityManagerFactory(
			EntityManagerFactory emFactoryParam) {
	...
	public static void setupTestClass() {
		...
		setEntityManagerFactory(Persistence
				.createEntityManagerFactory("entities-test"));
	...
	public void createDepartment() {
		 ...
		 * 
		 * logger.info("in createDepartment test Number of departments in the DB:"
		 * + beforeCount);
		 */
	...
	public void createDeptAndEmployee() {
		...
		logger.info("in createEmployee test Number of employees in the DB:"
				+ beforeCount);
		...
		for (Employee e : employeeList) {
			...
			logger.info("is employee instance managed? "
					+ getEntityManager().contains(employee));
			logger.info("is parking lot instance managed? "
					+ getEntityManager().contains(employee.getParking()));
		}
	...
	public void createEmployee() {
		...
		// because merge returns a entity of same type that is managed by em and
		// it contains the id.
		// persist acts on the same instance but merge copies the contents to a
		// managed object and returns it

		getEntityManager().getTransaction().commit();

		logger.info("employee id set on parking object : "
				+ park1.getEmp().getId());
		...
		for (Employee e : employeeList) {
			...
			logger.info("is employee instance managed? "
					+ getEntityManager().contains(employee));
			logger.info("is parking lot instance managed now ? "
					+ getEntityManager().contains(employee.getParking()));
	...
	public void createEmployeeWithParking() {
		...
		try {
			...
			// getId on employee will return correct value as it is set by the
			// em.
			// but if we use merge instead of persist, getId will return null
	...
	@Test
	public void thatProjectWithEmployeeQueryWorks() {
		
		//n+1 problem below. each getProject calls fires n requests to get parking details. project details is dealt with 
		//differently as it has join table 
		...
		// em.createQuery("select e from Employee e where e.projects IS NOT EMPTY",
		// Employee.class);
		// TypedQuery<Employee> query =
		// em.createQuery("select e from Employee e where e.department IS NOT NULL",
		// Employee.class);
		...
		for (Employee one : list) {
			...
			for (Project empProject : one.getProjects())
				System.out.println(empProject);

		}
		for (Employee one : list) {
			ParkingSpace ps = one.getParking();
